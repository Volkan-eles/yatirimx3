...
import { MOCK_STOCKS, MOCK_IPOS, MOCK_TARGET_PRICES } from '../constants';

const Home: React.FC = () => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filter, setFilter] = useState<'Tümü' | 'Yükselenler' | 'Hacim'>('Tümü');

  let displayedStocks = MOCK_STOCKS.filter(stock => 
        ...
        <div className="p-6 flex flex-col md:flex-row justify-between items-center gap-4 border-b border-white/[0.03]">
          ...
          <div className="flex gap-2 w-full md:w-auto">
            <button 
              ...
              className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-xs font-bold transition-all border flex items-center justify-center gap-2 ${
                filter === 'Yükselenler' 
              ...
              {displayedStocks.map((stock) => {
                ...
                return (
                  <tr key={stock.code} className="group hover:bg-white/[0.01] transition-colors">
                    ...
                    <td className="px-6 py-5">
                      <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[11px] font-black w-20 justify-center tabular-nums ${
                        isPositive 
